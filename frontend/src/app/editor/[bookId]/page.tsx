            ...
            .map(([chIdx, info]) => (
              <div key={chIdx}>
                ...
                {selectedChapter === +chIdx && (<>
                  ...
                  {/* Chapter Ending (was generated by the backend but had no UI entry) */}
                  {(() => {
                    const ce = specialPages.find(p => p.type === "chapter_ending" && p.chapter === +chIdx);
                    return ce ? (
                      <button
                        onClick={() => { setSelectedSegment(null); setSelectedSpecial(ce); }}
                        className={`w-full text-left px-6 py-2 text-xs border-b border-gray-50 transition-colors flex items-center gap-1.5 ${
                          selectedSpecial?.type === "chapter_ending" && selectedSpecial?.chapter === +chIdx
                            ? "bg-sky/20 text-gray-800" : "hover:bg-gray-50 text-gray-500"
                        }`}
                      >
                        <span className={`w-2 h-2 rounded-full shrink-0 ${ce.url ? "bg-green-400" : "bg-gray-300"}`} />
                        <span>Chapter Ending</span>
                      </button>
                    ) : null;
                  })()}
